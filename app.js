...
const { computeDHashHex, computeTileDHashes } = require("./utils/imageHash");
...
app.post('/api/upload', upload.single('image'), async (req, res) => {
    try {
        ...
        try {
            ...
            const tileHashes = await computeTileDHashes(filePath, 4); // 4x4 lưới

            // Lưu hash toàn ảnh (tile_index = -1)
            await db.execute(
                `INSERT INTO image_hashes (image_id, tile_index, grid, hash) VALUES (?, ?, ?, ?)`,
                [result.insertId, -1, 4, globalHash]
            );

            // Lưu hash cho từng tile
            if (Array.isArray(tileHashes)) {
                for (let i = 0; i < tileHashes.length; i++) {
                    await db.execute(
                        `INSERT INTO image_hashes (image_id, tile_index, grid, hash) VALUES (?, ?, ?, ?)`,
                        [result.insertId, i, 4, tileHashes[i]]
                    );
                }
            }
        } catch (hashErr) {
...
app.get('/api/search', async (req, res) => {
    try {
        ...
        query += ` ORDER BY id DESC LIMIT ${offsetNum >= 0 ? offsetNum + 0 : 0}, ${limitNum}`;
...
app.get('/api/image/:id', async (req, res) => {
    try {
        const [images] = await db.execute('SELECT * FROM images WHERE id = ?', [req.params.id]);
...
app.post('/api/reindex-hashes', async (req, res) => {
    try {
        ...
        for (const img of images) {
            const [exists] = await db.execute('SELECT COUNT(*) as c FROM image_hashes WHERE image_id = ?', [img.id]);
            if (exists[0].c > 0) continue;
            try {
                const globalHash = await computeDHashHex(img.file_path);
                const tileHashes = await computeTileDHashes(img.file_path, 4);
                await db.execute('INSERT INTO image_hashes (image_id, tile_index, grid, hash) VALUES (?, ?, ?, ?)', [img.id, -1, 4, globalHash]);
                for (let i = 0; i < tileHashes.length; i++) {
                    await db.execute('INSERT INTO image_hashes (image_id, tile_index, grid, hash) VALUES (?, ?, ?, ?)', [img.id, i, 4, tileHashes[i]]);
                }
                created++;
            } catch (e) {
...
app.post('/api/search-by-image', upload.single('image'), async (req, res) => {
    try {
        ...
        if (!buffer) {
            return res.status(400).json({ error: 'Vui lòng upload ảnh hoặc cung cấp url' });
        }

        // Hash ảnh truy vấn
        const queryHash = await computeDHashHex(buffer);

        // Lấy tất cả hash đã index
        ...
        const threshold = Number.isFinite(Number(req.query?.threshold)) ? Number(req.query.threshold) : 16;
        const topK = Number.isFinite(Number(req.query?.topK)) ? Number(req.query.topK) : 20;
        const filtered = results.filter(r => r.distance <= threshold).slice(0, topK);
    ...
    try {
        ...
        if (!queryHash) {
            try {
                ...
                await db.execute('INSERT INTO image_hashes (image_id, tile_index, grid, hash) VALUES (?, -1, 4, ?)', [imageId, queryHash]);
            } catch (e) {
