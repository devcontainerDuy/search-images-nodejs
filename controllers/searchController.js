...
async function searchText(req, res) {
  try {
    ...
    res.json({
      images: images.map(img => ({ ...img, url: `/uploads/images/${img.filename}`, thumbnail: `/uploads/images/${img.filename}` })),
      pagination: { current: pageNum, limit: limitNum, total, pages: Math.ceil(total / limitNum) },
    });
  } catch (e) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      client.get(url, (resp) => {
        ...
        if (resp.statusCode !== 200) { return reject(new Error('Failed to download image, status ' + resp.statusCode)); }
        const data = [];
...
async function searchByImage(req, res) {
  try {
      ...
      try {
        ...
        if (ann.isAvailable()) {
          ...
          if (annResults && annResults.length) {
            ...
            return res.json({ method: 'clip', model: clip.MODEL_ID, results: annResults.filter(r => r.similarity >= minSim).slice(0, topK), info: { totalIndexed: annResults.length, minSim, topK, ann: true } });
          }
        }
        // fallback scan
        ...
        const minSim = Number.isFinite(Number(req.query?.minSim)) ? Number(req.query.minSim) : 0.25;
        return res.json({ method: 'clip', model: clip.MODEL_ID, results: results.filter(r=>r.similarity>=minSim).slice(0, topK), info: { totalIndexed: rows.length, minSim, topK, ann: false } });
      } catch (clipErr) { console.error('CLIP search error, fallback to hash:', clipErr); }
    }
    ...
    let qColor = null;
    try { const c = await colors.computeColorHistogram(buffer); qColor = c.vector; } catch (_) {}
    ...
    for (const r of rows) {
      ...
      const entry = byImage.get(r.image_id) || { image_id: r.image_id, filename: r.filename, original_name: r.original_name, title: r.title, description: r.description, bestDistance: Infinity, bestTile: null };
      if (bestForRow < entry.bestDistance) { entry.bestDistance = bestForRow; entry.bestTile = r.tile_index; }
      byImage.set(r.image_id, entry);
    }
    const results = Array.from(byImage.values()).map(it => ({
      imageId: it.image_id,
    ...
    return res.json({ method: 'hash', query: { hash: globalHash }, results: filtered, info: { totalIndexed: byImage.size, threshold, topK } });
  } catch (e) {
    console.error('Search-by-image error:', e);
    res.status(500).json({ error: 'Lỗi server khi tìm kiếm theo hình ảnh' });
  }
}
...
async function similarById(req, res) {
  try {
    ...
    if (!queryHash) {
      ...
      } catch (e) { console.error('Failed computing/storing global hash for', imageId, e); return res.status(500).json({ error: 'Không thể tính hash cho ảnh' }); }
    }
    const threshold = Number.isFinite(Number(req.query?.threshold)) ? Number(req.query.threshold) : 20;
    const topK = Number.isFinite(Number(req.query?.topK)) ? Number(req.query.topK) : 20;
    const [rows] = await db.execute(`
    ...
    for (const r of rows) {
      ...
      const entry = byImage.get(r.image_id) || { image_id: r.image_id, filename: r.filename, original_name: r.original_name, title: r.title, description: r.description, bestDistance: Infinity, bestTile: null };
      if (d < entry.bestDistance) { entry.bestDistance = d; entry.bestTile = r.tile_index; }
      byImage.set(r.image_id, entry);
    }
    const results = Array.from(byImage.values()).map(it => ({
      imageId: it.image_id, url: `/uploads/images/${it.filename}`, filename: it.filename,
      original_name: it.original_name, title: it.title, description: it.description,
      distance: it.bestDistance, similarity: Number((1 - it.bestDistance / 64).toFixed(4)), matchedTile: it.bestTile,
    })).filter(r => r.distance <= threshold).sort((a,b)=> a.distance - b.distance).slice(0, topK);
    res.json({ query: { imageId, hash: queryHash }, results, info: { threshold, topK } });
  } catch (e) {
